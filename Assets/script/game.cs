...
public class game : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        gamer_list[1].set_delta_gold();//покажем деньги игрока
        ...
        if (!data.get_flag_army_is_move())//если нет армий в пути сможем завершить ход
        {
            ...
            while (count < 20)//ищем очередного игрока
            {
                ...
                if (gamer_list[next].still_play)
                {
                    ...
                    foreach (city c in gamer_list[next].city_list) c.collect_profit();//собираем в казну деньги с каждого города
                    gamer_list[next].collect_cost();//соберем деньги за содержание армии
                    //итерация производства юнитов во всех городах игрока
                    foreach (city c in gamer_list[next].city_list) c.create_unit();
                    gamer_list[next].set_delta_gold();//обновим стоимость содержания армии
                    break;
                }
