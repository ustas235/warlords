...
public class gamer //
{
    ...
    {//получение стоимости содержания войск
        float cost = 0f;
        foreach (s_army a in s_army_list)
        {
            foreach (unit u in a.get_unit_list())
            {
                cost = cost + (u.price * data.koef_cost);//считаем стоимость содержания всех юнитов
            }
        }
        return cost;
    }
    public void delete_extra_unit()
    {
        ...
        if (extra_cost<0)
        {
            foreach (s_army a in s_army_list)
                foreach (unit u in a.get_unit_list())
                { 
                    all_unit_list.Add(u);//собираем всех юнитов в один спискок
                }
            ...
            {// удаляем лишние юниты
                ...
                if (gold - cost < 0)
                {
                    ...
                    tmp_u.sc_army.sub_unit_destroy(tmp_u);//удаляем последний юнит
                }
                else break;
            }
        }
